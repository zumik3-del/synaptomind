...
- timeline: List thoughts with pagination (recent activity feed)`,
    inputSchema: {
      ...
      pending: z.array(z.string()).optional().describe('Pending tasks — each creates a draft thought with tag "pending" that surfaces in the frontier after wake_days (reflect only)'),
      wake_days: z.number().int().min(1).max(365).optional().describe('Days before pending items surface in the frontier (default 7, 1-365, reflect only)'),
