...
> Measurements were taken on v0.5.0 as released. Current release: 0.7.0-beta.0. The default embedder interval (`embedder.pollIntervalMs`) is now 7000 ms (7s) — the "interval=3s" row below reflects the configuration used at measurement time, not the current default.
